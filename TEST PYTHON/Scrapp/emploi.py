...
def get_job_description(link, session):
    try:
        resp = session.get(link, timeout=50)
        soup = BeautifulSoup(resp.text, 'html.parser')
        desc_tag = soup.select_one('.job-description')
        if not desc_tag:
            desc_tag = soup.select_one('.section-job-description')  # fallback
        return desc_tag.get_text(strip=True) if desc_tag else None
    except Exception as e:
        logging.error(f"❌ Error fetching description from {link}: {e}")
        return None

def extract_jobs(soup, session):
    job_list = []
    ...
    for card in wrapper.select('.card.card-job'):
        try:
            ...
            description = get_job_description(link, session) if link else None
...
def scrape_all_jobs():
    ...
    for page in range(1, total_pages + 1):
        ...
        try:
            resp = session.get(page_url, timeout=50)
            page_soup = BeautifulSoup(resp.text, 'html.parser')
            jobs = extract_jobs(page_soup, session)
