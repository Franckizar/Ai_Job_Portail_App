...
def call_ai_model(messages: List[Dict[str, str]]) -> Optional[str]:
    """Call the AI model using /api/chat endpoint with conversation context"""
    try:
...
def ai_response():
    """Main AI endpoint for processing user questions with conversation context"""
    try:
        ...
        # Add the SQL generation prompt
        messages.append({"role": "user", "content": strict_prompt})
        
        generated_sql = call_ai_model(messages)
        ...
        # Step 5: Generate natural language response with conversation context
        data_summary = generate_data_summary(rows, columns)
        
        # Create messages for natural response generation
        response_messages = []
        
        # Add conversation history for context
        if conversation_history:
            response_messages.extend(conversation_history)
        
        # Add the natural response prompt
        response_messages.append({
            "role": "user", 
            "content": generate_natural_response_prompt(user_question, data_summary)
        })
        
        final_answer = call_ai_model(response_messages)
        
        if not final_answer:
            final_answer = "I've found the information you requested. Please check the data below."
...
def generate_natural_response_prompt(user_question: str, data_summary: str) -> str:
    """Generate prompt for natural response generation"""
...
def test_endpoint():
    ...
    return jsonify({
        "message": "Enhanced AI SQL Server with Chat API is running!",
        "database": DB_CONFIG["database"],
        "api_endpoint": AI_CONFIG["url"],
        "features": [
...
def health_check():
    ...
    try:
        response = requests.get(AI_CONFIG["url"].replace('/api/chat', '/api/tags'), timeout=5)
...
if __name__ == '__main__':
    print("🚀 Enhanced AI SQL Server with Chat API and conversation context")
    print(f"📊 Database: {DB_CONFIG['database']}")
    print(f"🤖 AI Endpoint: {AI_CONFIG['url']}")
    ...
    print("✅ Using ACTUAL database schema with chat API conversation context!")
